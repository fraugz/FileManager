...
### Multi-Selection — Available Actions

Visible actions in the bottom bar depend on the active tab and what is selected.

**Storage**

| Selection | Send | Open | Play | Pin/Unpin | Move | Copy | Delete | Rename | Info |
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 1 file | ✓ | ✓ | — | — | ✓ | ✓ | ✓ | ✓ | ✓ |
| 1 folder | — | — | — | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| N files (all media) | ✓ | — | ✓ | — | ✓ | ✓ | ✓ | — | — |
...
| Selection | Send | Play | Locate | Move | Copy | Delete | Rename | Pin/Unpin |
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 1 file | ✓ | — | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| 1 folder | — | — | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| N files (all media) | ✓ | ✓ | — | ✓ | ✓ | ✓ | — | ✓ |
...
| Selection | Restore | Info | Delete forever |
|---|:---:|:---:|:---:|
| 1 item | ✓ | ✓ | ✓ |
| N items | ✓ | — | ✓ |
...
- **v1.3.2:** Critical fix: files now move to app trash instead of being deleted directly. Fixed multi-selection also deleting permanently. Multi-selection in Trash screen (long-press to select, tap to toggle, batch Restore/Delete actions). Trash thumbnails now use original filename for media type detection. Visited folders now appear in Recents sorted by last access.
- **v1.3.1:** Trash screen UX overhaul: thumbnail previews for image/video/audio/APK items; click opens file with the preferred app (honouring DefaultAppsManager, same as main browser); long-press shows a bottom action bar (Restore / Info / Delete) with icons matching the main selection bar; both app-trash and system-trash sections are always visible with an empty-state placeholder when each is empty; section headers carry a ? info badge (theme-aware); Empty trash covers both sections with a combined count confirmation dialog; folder item counts now exclude hidden files and flag trashed items separately; fixed image opening via MediaStore URI so gallery apps can swipe between siblings; auto-delete countdown shown in item rows and Info dialog; Info dialog also shows the original file path; typo fix in Spanish "Tamaño total en papelera".
